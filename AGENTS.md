...
```
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -scheme LiteBudget -destination 'platform=macOS' build
```
...
```
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -scheme LiteBudget -destination 'platform=macOS' build 2>&1 | grep -E "error:|BUILD"
```